...
The bug review has validated fixes for web/native **0.6.3**, Android versionCode **9**, and extension **0.1.2**. Commit/push, the GitHub signed APK build, and release publication are the remaining steps. The previous published APK is 0.6.2 and the previous extension is 0.1.1; do not overwrite those release assets.
...
Finish the validated bug-fix release: web/native 0.6.3 (Android versionCode 9) and extension 0.1.2. Check current Git/GitHub state first to avoid duplicate builds or releases. Commit/push the fixes, build the signed APK on GitHub, verify its checksum and unchanged signing identity, publish matching APK/extension assets, and verify the live About/installation links. Record evidence and limitations and update this continuation prompt. Do not start Milestone 7. Use the emulator, and notify me before any necessary wireless-phone test. Never request signing secrets or recovery keys in chat; no Drive reconnection is needed.
```
