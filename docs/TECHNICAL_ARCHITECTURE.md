...
Per D-028, `packages/sync-http` implements the same `SyncObjectStore` boundary as Drive over an authenticated HTTPS API. The client stores the endpoint and account identity locally; passwords are never persisted. An opted-in trusted browser device encrypts its PocketBase token and master-key material under a non-exportable WebCrypto device key in IndexedDB, refreshes the token on startup, and removes both records on disconnect. The recovery key itself is not retained. Plain HTTP is rejected except on loopback test hosts.

`servers/pocketbase` is the first adapter. PocketBase 0.40.4 migrations use a closed, shared `users` auth collection and a private PostKeeper object collection. This separates server identity from any one application while keeping every application's storage and routes distinct. Hooks provide paginated prefix listing, byte reads with ETags, immutable creation, atomic conditional updates, per-object limits and per-user quotas. New PostKeeper libraries use an opaque `libraries/<library-id>/` path namespace; legacy recovery keys retain the root layout. The server receives opaque encrypted bytes and never parses article content or receives website credentials or recovery keys.

Normal sync lists the library's deterministic encrypted object paths once, uploads only missing blobs and operations, and downloads only operation paths absent from the local log. An unchanged library therefore performs a metadata validation read and an object listing without replaying ciphertext or rebuilding local state. Trusted devices check after startup, shortly after local edits, and hourly while open; an explicit **Check now** action provides an immediate pull/push without foreground-triggered polling.
