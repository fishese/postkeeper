...
An unlocked trusted device checks once after PostKeeper opens, shortly after a local library change, and about once an hour while the app remains open. Returning to the foreground does not itself start another check. **Check now** performs an immediate bidirectional check when a change was made on another device. Established libraries list deterministic encrypted object paths and transfer only missing immutable blobs and operations; an unchanged check does not replay the library.
