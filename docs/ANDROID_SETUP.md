...
Local 2026-09-09 follow-up (not yet published): **Browser options → Save full page (if article text is missing)** saves a sanitized copy of already-loaded page content when article extraction omits text. It can include extra page text and cannot recover content withheld by a login/subscription page. Normal Save page now tries substantial semantic article content when Readability returns only a short teaser. Selected responsive/lazy image URLs are preserved. Native capture downloads public cross-origin images and follows at most five validated redirects; cookies remain limited to exact original same-origin candidates. Images from other origins requiring authentication may remain unavailable.

Article details now offers **Delete article** and **Remove images from reading copy**, each with confirmation. Deletion hides the article and synchronizes its tombstone on the next explicit sync. Image removal retains text in a new current snapshot. Historical snapshots, shared blobs and existing backups remain, so these are not secure-erasure/storage-reclamation controls.
...
- Capture reuses credential-scrubbed DOM and Readability extraction. Native image downloads use that profile's cookies only for exact image candidates on the current page origin. Public cross-origin candidates and validated redirects can be downloaded without those cookies; other origins' cookie stores are never read. Native networking never accepts a website-selected arbitrary bridge action.
