...
describe('extension installation document', () => {
  it('is a script-free public page with versioned downloads and wrapper guidance', () => {
    ...
    expect(
      document.querySelector('a[href$="/extension-v0.1.4/postkeeper-chromium-0.1.4.zip"]'),
    ).not.toBeNull();
    expect(
      document.querySelector('a[href$="/extension-v0.1.4/postkeeper-firefox-0.1.4.zip"]'),
    ).not.toBeNull();
    expect(document.body.textContent).not.toMatch(/TODO|\[your|placeholder/iu);
  });
});
